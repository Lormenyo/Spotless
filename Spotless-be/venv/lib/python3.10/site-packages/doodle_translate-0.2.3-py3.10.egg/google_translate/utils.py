...
def make_request(url, headers=None, proxy=None, timeout=10.0, simulate=False):
    ...
    Raises:
        urllib3.HTTPError, urllib3.URLError, IOError
    ...
    if proxy is None:
        ...
        url_opener = urllib3.build_opener(urllib3.ProxyHandler({"http": proxy, "https": proxy}))
    ...
    except (urllib3.HTTPError, urllib3.URLError, IOError) as error:
        raise error
    finally:
        url_opener.close()
...
def quote_unicode(text, encoding="utf-8"):
    """urllib3.quote wrapper to handle unicode items."""
    if isinstance(text, unicode):
        text = text.encode(encoding)

    return urllib3.quote(text).decode(encoding)


def unquote_unicode(text, encoding="utf-8"):
    """urllib3.unquote wrapper to handle unicode items."""
    if isinstance(text, unicode):
        text = text.encode(encoding)

    return urllib3.unquote(text).decode(encoding)
